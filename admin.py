...
def get_tables(cursor, operation):
    ...
    if operation == "Update data":
        relevant_tables = ['accident', 'inspection', 'insurance', 'license',
                           'maintenance', 'owners', 'registration', 'vehicle', 'violation']
    ...
    if table == 'accident':
        ...
        if owner_id:
            ...
            if vehicles:
                ...
                vehicle_options = [
                    f"{vehicle[0]} - {vehicle[1]}" for vehicle in vehicles]

                # Allow admin to choose from the list of vehicles
                selected_vehicle = st.selectbox(
                    'Select Vehicle', vehicle_options)
                ...
                add_cols = [acc_id, license_num, vehicle_id,
                            location, acc_date, acc_desc]
                ...
                st.warning(
                    f'No vehicles found for the owner with ID: {owner_id}')
        else:
            if license_num:
                st.warning(
                    f'No owner found for the given license number: {license_num}')
        ...
        add_cols = [violation_id, Vehicle_id,
                    fine_amt, violation_date, violation_type]
...
def delete_data(db, cursor):
    ...
    if vehicle_ids:
        selected_vehicle_id = st.selectbox(
            'Select Vehicle ID to Delete', vehicle_ids)
        ...
        if st.button('Delete Vehicle Data'):
            try:
                ...
                st.success(
                    f'Data for Vehicle ID {selected_vehicle_id} deleted successfully!')
                db.commit()
...
def update_data(db, cursor, table, column):
    ...
    # Remove empty values
    update_options = {k: v for k, v in update_options.items() if v}

    # Update the record
    if st.button('Update Data'):
        try:
            set_clause = ", ".join(
                [f"{col} = %s" for col in update_options.keys()])
            ...
            db.commit()
            st.success(
                f'Data for {table} ID {record_id} updated successfully!')
        except Exception as e:
            st.warning(f'Error: {e}')
...
def insert_new_details(db, cursor):
    ...
    owner_data = [owner_id, owner_name, owner_phone,
                  license_num, owner_mail, gender, address, dob]
    ...
    vehicle_data = [Vehicle_id, RegNumber, model, owner_id,
                    RegDate, NumberPlate, VehicleType, Vehicle_description]
    ...
    result = st.text_input(
        'Result of the Inspection (1 for Positive result and 0 for Negative result)')
...
def analyze(cursor):
    ...
    sns.barplot(x='Gender', y='Count', data=df, linewidth=2,
                edgecolor="0.2", color='skyblue')
    ...
    # Plot the bar chart
    plt.figure(figsize=(4, 2))
    sns.barplot(x='Month', y='Count', data=accidents_by_month,
                linewidth=2, edgecolor="0.2", color='skyblue')
    ...
    # Plot the bar chart
    plt.figure(figsize=(4, 2))
    sns.barplot(x='Month', y='Count', data=violations_by_month,
                linewidth=2, edgecolor="0.2", color='skyblue')
...
def admin_dashboard():
    ...
    if authenticate_admin(username, password):
        ...
        option = st.sidebar.selectbox(
            'Select Operation', ['View data', 'Insert data', 'Update data', 'Delete data', 'Analyze data', 'Execute a query'])
